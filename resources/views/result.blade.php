<x-app-layout>
    ...
    function getAnalisisGap($nilai_user, $bidang) {

        $kekurangan = [];

        if($nilai_user['hardskill'] < 3.5) $kekurangan[] = "penguasaan alat teknis/hardskill";

        if($nilai_user['minat'] < 3.5) $kekurangan[] = "minat mendalam pada bidang ini";

        if($nilai_user['pengalaman'] < 3) $kekurangan[] = "jam terbang atau portofolio proyek";

       

        if(empty($kekurangan)) return "Profil kamu sudah cukup baik, namun perlu pengembangan berkelanjutan agar lebih bersaing.";

        return "Skor ini dipengaruhi oleh terbatasnya " . implode(', ', $kekurangan) . ". Disarankan untuk fokus memperdalam kriteria tersebut agar tingkat kecocokan meningkat.";
