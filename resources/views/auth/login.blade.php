    ...
    <div class="flex min-h-screen">
        ...
        <div class="w-full md:w-1/2 flex items-center justify-center p-8 sm:p-16">
            <div class="w-full max-w-lg">
                <h2 class="text-4xl font-extrabold text-[#1E293B] mb-3">Selamat datang kembali</h2>
                <p class="text-lg text-gray-500 mb-10 font-medium">Masuk ke akun Career Insight kamu.</p>
                ...
                <form method="POST" action="{{ route('login') }}" class="space-y-6">
                    ...
                    <div>
                        <label for="login" class="block text-base font-bold text-[#1E293B] mb-2">Email atau NIM</label>
                        <input id="login" type="text" name="login" value="{{ old('login') }}" required autofocus class="w-full px-5 py-4 text-lg rounded-xl border border-gray-200 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-[#4298B4] focus:border-transparent transition-all" placeholder="Masukkan NIM atau email">
                        <x-input-error :messages="$errors->get('login')" class="mt-2 text-red-500 text-base" />
                    </div>

                    <div>
                        <label for="password" class="block text-base font-bold text-[#1E293B] mb-2">Kata Sandi</label>
                        <input id="password" type="password" name="password" required class="w-full px-5 py-4 text-lg rounded-xl border border-gray-200 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-[#4298B4] focus:border-transparent transition-all" placeholder="Masukkan kata sandimu">
                        <x-input-error :messages="$errors->get('password')" class="mt-2 text-red-500 text-base" />
                    </div>

                    <button type="submit" class="w-full bg-[#4298B4] hover:bg-[#327A94] text-white font-bold py-4 rounded-xl transition-colors shadow-lg shadow-[#4298B4]/30 mt-6 text-xl">
                        Masuk
                    </button>
                </form>

                <p class="text-center text-base text-gray-500 mt-10 font-medium">
                    Belum punya akun?
                    <a href="{{ route('register') }}" class="text-[#4298B4] font-bold hover:underline">Daftar Sekarang</a>
                </p>
            </div>
        </div>
    </div>
